...
5. **Publish** — `npm publish ./dist` with `NODE_AUTH_TOKEN` set to `GITHUB_TOKEN`. Publishing from `dist/` directly means consumers resolve against [ng-packagr ↗](https://github.com/ng-packagr/ng-packagr)'s generated `package.json` with the correct [APF ↗](https://angular.dev/tools/libraries/angular-package-format) entry points.
...
```json
{
    ...
    "packages": {
        ".": {
            "package-name": "@teqbench/tbx-mat-icons",
